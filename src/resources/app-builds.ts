...
export class AppBuilds extends APIResource {
  ...
  retrieve(
    id: string,
    params: AppBuildRetrieveParams | null | undefined = {},
    options?: RequestOptions,
  ): APIPromise<Shared.AppBuild> {
    const { 'Api-Version-Date': apiVersionDate } = params ?? {};
    return this._client.get(path`/app_builds/${id}`, {
      ...options,
      headers: buildHeaders([
        { ...(apiVersionDate != null ? { 'Api-Version-Date': apiVersionDate } : undefined) },
        options?.headers,
   ...
   * for await (const appBuild of client.appBuilds.list({
   *   app_id: 'app_id',
   * })) {
   *   // ...
   * }
...
export interface AppBuildRetrieveParams {
  /**
