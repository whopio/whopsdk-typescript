...
describe('resource wallets', () => {
  ...
  // Mock server tests are disabled
  test.skip('signTransaction: only required params', async () => {
    const responsePromise = client.wallets.signTransaction('account_id', { chain_id: 0, to: 'to' });
    const rawResponse = await responsePromise.asResponse();
    expect(rawResponse).toBeInstanceOf(Response);
    const response = await responsePromise;
    expect(response).not.toBeInstanceOf(Response);
  ...
  // Mock server tests are disabled
  test.skip('signTransaction: required and optional params', async () => {
    const response = await client.wallets.signTransaction('account_id', {
      chain_id: 0,
      to: 'to',
      data: 'data',
      value: 'value',
