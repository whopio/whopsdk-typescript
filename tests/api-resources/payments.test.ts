...
describe('resource payments', () => {
  ...
  test.skip('listFees: request options and params are passed correctly', async () => {
    // ensure the request options are being passed correctly by passing an invalid HTTP method in order to cause an error
    await expect(client.payments.listFees('pay_xxxxxxxxxxxxxx', {
    after: 'after',
    before: 'before',
    first: 42,
    last: 42,
  ...
  test.skip('refund: request options and params are passed correctly', async () => {
    // ensure the request options are being passed correctly by passing an invalid HTTP method in order to cause an error
    await expect(client.payments.refund('pay_xxxxxxxxxxxxxx', { partial_amount: 6.9 }, { path: '/_stainless_unknown_path' }))
      .rejects
      .toThrow(Whop.NotFoundError);
  });
