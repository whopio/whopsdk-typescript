...
describe('resource authorizedUsers', () => {
  ...
  test.skip('list: request options and params are passed correctly', async () => {
    ...
    await expect(client.authorizedUsers.list({
    after: 'after',
    before: 'before',
    company_id: 'biz_xxxxxxxxxxxxxx',
  ...
  test.skip('delete: request options and params are passed correctly', async () => {
    // ensure the request options are being passed correctly by passing an invalid HTTP method in order to cause an error
    await expect(client.authorizedUsers.delete('ausr_xxxxxxxxxxxxx', { company_id: 'biz_xxxxxxxxxxxxxx' }, { path: '/_stainless_unknown_path' }))
      .rejects
      .toThrow(Whop.NotFoundError);
  });
});
