...
const EMBEDDED_METHODS: MethodEntry[] = [
  ...
  },
  {
    ...
    response:
      "{ id: string; api_version: 'v1' | 'v2' | 'v5'; child_resource_events: boolean; created_at: string; enabled: boolean; events: string[]; resource_id: string; url: string; webhook_secret: string; }",
    markdown:
      "## list\n\n`client.webhooks.list(company_id: string, after?: string, app_id?: string, before?: string, first?: number, last?: number): { id: string; api_version: api_version; child_resource_events: boolean; created_at: string; enabled: boolean; events: webhook_event[]; resource_id: string; url: string; webhook_secret: string; }`\n\n**get** `/webhooks`\n\nReturns a paginated list of webhook endpoints configured for a company, ordered by most recently created.\n\nRequired permissions:\n - `developer:manage_webhook`\n\n### Parameters\n\n- `company_id: string`\n  The unique identifier of the company to list webhooks for.\n\n- `after?: string`\n  Returns the elements in the list that come after the specified cursor.\n\n- `app_id?: string`\n  Only return webhooks attached to this app. Omit to list the company's own webhooks.\n\n- `before?: string`\n  Returns the elements in the list that come before the specified cursor.\n\n- `first?: number`\n  Returns the first _n_ elements from the list.\n\n- `last?: number`\n  Returns the last _n_ elements from the list.\n\n### Returns\n\n- `{ id: string; api_version: 'v1' | 'v2' | 'v5'; child_resource_events: boolean; created_at: string; enabled: boolean; events: string[]; resource_id: string; url: string; webhook_secret: string; }`\n  A webhook endpoint that receives event notifications for a company via HTTP POST.\n\n  - `id: string`\n  - `api_version: 'v1' | 'v2' | 'v5'`\n  - `child_resource_events: boolean`\n  - `created_at: string`\n  - `enabled: boolean`\n  - `events: string[]`\n  - `resource_id: string`\n  - `url: string`\n  - `webhook_secret: string`\n\n### Example\n\n```typescript\nimport Whop from '@whop/sdk';\n\nconst client = new Whop();\n\n// Automatically fetches more pages as needed.\nfor await (const webhookListResponse of client.webhooks.list({ company_id: 'biz_xxxxxxxxxxxxxx' })) {\n  console.log(webhookListResponse);\n}\n```",
    perLanguage: {
      typescript: {
  ...
  },
  {
    ...
    response:
      "{ id: string; api_version: 'v1' | 'v2' | 'v5'; child_resource_events: boolean; created_at: string; enabled: boolean; events: string[]; resource_id: string; testable_events: string[]; url: string; webhook_secret: string; }",
    markdown:
      "## retrieve\n\n`client.webhooks.retrieve(id: string): { id: string; api_version: api_version; child_resource_events: boolean; created_at: string; enabled: boolean; events: webhook_event[]; resource_id: string; testable_events: webhook_event[]; url: string; webhook_secret: string; }`\n\n**get** `/webhooks/{id}`\n\nRetrieves the details of an existing webhook.\n\nRequired permissions:\n - `developer:manage_webhook`\n\n### Parameters\n\n- `id: string`\n\n### Returns\n\n- `{ id: string; api_version: 'v1' | 'v2' | 'v5'; child_resource_events: boolean; created_at: string; enabled: boolean; events: string[]; resource_id: string; testable_events: string[]; url: string; webhook_secret: string; }`\n  A webhook endpoint that receives event notifications for a company via HTTP POST.\n\n  - `id: string`\n  - `api_version: 'v1' | 'v2' | 'v5'`\n  - `child_resource_events: boolean`\n  - `created_at: string`\n  - `enabled: boolean`\n  - `events: string[]`\n  - `resource_id: string`\n  - `testable_events: string[]`\n  - `url: string`\n  - `webhook_secret: string`\n\n### Example\n\n```typescript\nimport Whop from '@whop/sdk';\n\nconst client = new Whop();\n\nconst webhook = await client.webhooks.retrieve('hook_xxxxxxxxxxxxx');\n\nconsole.log(webhook);\n```",
    perLanguage: {
      typescript: {
