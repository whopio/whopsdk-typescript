  ...
  inputSchema: {
    ...
    properties: {
      ...
      void_payments: {
        type: 'boolean',
        description:
          'Whether to void past_due payments associated with the membership to prevent future payment attempts.',
      },
      jq_filter: {
        type: 'string',
        title: 'jq Filter',
...
export const handler = async (client: Whopsdk, args: Record<string, unknown> | undefined) => {
  ...
  return asTextContentResult(await maybeFilter(jq_filter, await client.memberships.pause(id, body)));
};
